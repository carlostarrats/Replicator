...
- Fake API and fixture test coverage.
...
- Dogfood workflow: add a single scripted local flow that chains report generation, policy checks, snapshots, CI, templates, handoff package, and viewer generation through the fake API.
...
- Proceed with the four follow-up implementation steps above using only local files and fake API servers.
